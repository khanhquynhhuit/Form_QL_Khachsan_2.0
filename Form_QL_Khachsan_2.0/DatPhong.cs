    ...
    public partial class DatPhong : Form
    {
        ...
        private void TaoHoaDonPdf(int hoaDonId, string hoten, string maPhong, DateTime ngayNhan, DateTime ngayTra, string emailKhach)
        {
            try
            {
                ...
                string tempPath = Path.Combine(pdfDir, $"Temp_{hoaDonId}.pdf"); // File chưa ký
                string finalPath = Path.Combine(pdfDir, $"HoaDon_{hoaDonId}_{timeStamp}.pdf"); // File đã ký
                ...
                // B. KÝ SỐ
                byte[] keyData = LayKeyTuOracle();
                if (keyData != null)
                {
                    try
                    {
                        KySoPdf(tempPath, finalPath, keyData, "123");
                        if (File.Exists(tempPath)) File.Delete(tempPath); // Xóa file tạm
                    }
                    catch (Exception ex)
                    {
                        MessageBox.Show("Lỗi ký số: " + ex.Message);
                        // Nếu ký lỗi thì dùng file tạm làm file chính
                ...
                {
                    // Không có key thì đổi tên file tạm thành file chính
                    if (File.Exists(tempPath)) File.Move(tempPath, finalPath);
                }

                // C. GỬI EMAIL
                if (!string.IsNullOrEmpty(emailKhach))
                {
                    GuiEmailKhachHang(emailKhach, "Hóa Đơn Điện Tử", $"Xin chào {hoten}, đính kèm là hóa đơn của bạn.", finalPath);
                    MessageBox.Show($"Đã xuất hóa đơn và gửi Email tới: {emailKhach}", "Thành công");
                }
                else
                {
            ...
            {
                MessageBox.Show("Lỗi quy trình hóa đơn: " + ex.Message);
            }
        }
        ...
        private void LoadPhongTrong()
        {
            try
            {
                ...
                {
                    while (reader.Read()) { comboxphong.Items.Add(reader["MAPH"].ToString() + " - " + reader["LOAIPHONG"].ToString()); }
                }
                if (comboxphong.Items.Count > 0) comboxphong.SelectedIndex = 0;
                else MessageBox.Show("Hết phòng!", "Thông báo");
            }
